...
void setup() {
    ...
    // ── Seed the scheduler — mirrors Pico's startup task chain ───────────────
    gScheduler.schedule(Tasks::init_ap);
    // init_ap → init_web_server → build_default_config (chained inside tasks)
}

// ─────────────────────────────────────────────────────────────────────────────
// loop()
// Mirrors the Pico's main loop exactly:
//   1. Drain the task scheduler.
//   2. Process all queued events.
//   3. Periodic checks (AP clients, stale data).
// ─────────────────────────────────────────────────────────────────────────────
void loop() {
    // ── 1. Run one pending task ───────────────────────────────────────────────
    if (!gScheduler.empty()) {
        TaskScheduler::TaskFunc task = gScheduler.pop();
        if (task) task(gCtx);
    }
    ...
    // ── 3. Periodic: check for stale Pico data ───────────────────────────────
    ...
    // ── 4. Periodic: log AP client count ─────────────────────────────────────
    if (now - lastApCheck >= AP_CHECK_INTERVAL) {
        lastApCheck = now;
        gScheduler.schedule(Tasks::await_pico_connection);
    }

    // ── 5. Clean up dead WebSocket connections ────────────────────────────────
    gWs.cleanupClients();
