...
struct AppContext {
    // Networking
    AsyncWebServer *server      = nullptr;   // HTTP + WebSocket host
    AsyncWebSocket *ws          = nullptr;   // /ws endpoint

    // Scheduler + Event queue
    TaskScheduler              *scheduler   = nullptr;
    EventQueue<24>             *events      = nullptr;

    // Shared state
    SystemConfig                config;          // current plant config
    PlantStatus                 lastStatus;      // most recent data from Pico
